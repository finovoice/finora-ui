  ...
  useEffect(() => {
    const fetchRms = async () => {
      try {
        ...
        setRms(data);
        if (data.length > 0) setAssigned_RM(data[0].email);
  ...
  const processClients = async (clients: EditableClient[]) => {
    ...
    try {
      ...
      showToast({
        title: "Error",
        description:
          "Failed to import leads via API. Check console for details.",
        type: "error",
        duration: 3000,
      });
      ...
      <DialogContent className="max-w-3xl w-3xl p-0 overflow-hidden rounded-xl">
        ...
        <Tabs defaultValue="manual" className="w-full">
          <TabsList className="grid w-full grid-cols-2 h-auto bg-white p-0 border-b border-[#e4e7ec] rounded-none">
            <TabsTrigger
              value="manual"
              className="rounded-none border-b-1.5 border-transparent data-[state=active]:border-b-[#7f56d9] data-[state=active]:shadow-none text-sm font-sm text-[#667085] data-[state=active]:text-[#7f56d9] py-3"
            >
              Manual
            </TabsTrigger>
            <TabsTrigger
          ...
          <TabsContent value="manual" className="p-6 space-y-5">
            ...
            <div className="flex flex-row gap-4 justify-between w-full">
              <div className="space-y-1 w-full">
                ...
                <Select
                  defaultValue={assigned_RM}
                  value={assigned_RM}
                  onValueChange={setAssigned_RM}
                >
              ...
              <div className="flex flex-col space-y-1.5">
                ...
                <div className="flex gap-2">
                  <Button
                    type="button"
                    variant={risk === "CONSERVATIVE" ? "default" : "outline"}
                  ...
                  <Button
                    type="button"
                    variant={risk === "AGGRESSIVE" ? "default" : "outline"}
            ...
            <div className="border-t border-[#e4e7ec] px-6 -mb-2 pt-6  flex flex-col gap-4 items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <Label
                  htmlFor="add-continuously"
                  className={`text-sm ${
                    addContinuously ? "text-[#475467]" : "text-gray-400"
                  } `}
                >
                  Add continuously
                </Label>
          ...
          <TabsContent value="bulk" className="p-5  space-y-5">
            {uploadState === "initial" && (
              <>
                <div className="rounded-lg border border-[#e4e7ec] bg-[#fcfcfd] p-5 space-y-2">
                  <h3 className="text-md font-medium text-[#101828]">
                    Instructions
                  </h3>
                  <p className="text-sm text-[#667085]">
                    Drop a csv file with the following columns -
                  </p>
                  <ol className="list-decimal list-inside text-sm text-[#667085] space-y-1">
                    <li>
                      <span className="font-semibold">Name *</span> - name of
                      the client
                    </li>
                    <li>
                      <span className="font-semibold">Country code *</span> -
                      country code if the client is outside; leave blank for
                      Indian numbers
                    </li>
                    <li>
                    ...
                    <li>
                      ...
                      plan subscribed which exactly matches your plans added in
                      the plans page
                    </li>
                    <li>
                      <span className="font-semibold">Plan expiry date </span>
                      write when plan expires in YYYY/MM/DD format; a day of 30
                      days from today will be added if left blank
                    </li>
                    <li>
                      <span className="font-semibold">Assigned RM</span> - write
                      name of RM which exactly matches the name added by you in
                      the employee page; Admin's name will be added if left
                      blank. for example ( admin@finora.com )
                    </li>
                    <li>
                      <span className="font-semibold">Risk Profile *</span>-
                      Choose one of the three options from "conservative",
                      "high", "aggressive"
                    </li>
                  </ol>
            ...
            {uploadState === "warning" && (
              <div className="pt-1 px-3 space-y-2 text-left">
                ...
                <p className="text-sm text-[#667085]">
                  Some rows were not added due to either or both of the reasons
                  stated below -
                </p>
                ...
                <p className="text-sm text-[#667085] pb-8">
                  On proceeding, only rows passing these conditions will be
                  added.
                </p>
            ...
            {uploadState === "error" && (
              <div className="pt-0 px-3 space-y-2 text-left">
                ...
                <p className="text-sm text-[#667085]">
                  All rows failed to add due to either or both of the reasons
                  stated below -
                </p>
                <ol className="list-decimal list-inside text-sm text-[#667085] space-y-1 text-left">
                  <li>Missing/invalid information in the required fields</li>
