...
export default function ClientsPage() {
  ...
  const planOptions: ("Elite" | "Standard" | "Premium")[] = [
    "Elite",
    "Standard",
    "Premium",
  ];
  ...
  const renewalOptions: (
    ...
    | "30 days"
    | "90 days or more"
  )[] = ["7 days", "14 days", "30 days", "90 days or more"];
    ...
    if (selectedPlan.length > 0) {
      currentClients = currentClients.filter(
        (client) =>
          ...
          selectedPlan
            .map((p) => p.toUpperCase())
            .includes(client.plan.toUpperCase())
      );
    }
    ...
    if (selectedRiskProfile.length > 0) {
      currentClients = currentClients.filter(
        (client) =>
          ...
          selectedRiskProfile
            .map((r) => r.toUpperCase())
            .includes(client.risk.toUpperCase())
      );
    }

    if (selectedRenewal) {
      // Check if a renewal option is selected
              ...
              <div className="flex items-center gap-3">
                ...
                <Tabs defaultValue="active">
                  <TabsList className="bg-transparent p-0">
                    <TabsTrigger
                      value="active"
                      className="rounded-md bg-[#f9f5ff] px-2 py-1 text-xs font-medium text-[#6941c6] data-[state=active]:shadow-none"
                    >
                      Active
                    </TabsTrigger>
                    <TabsTrigger
                      value="churned"
                      className="rounded-md px-2 py-1 text-xs text-[#667085]"
                    >
                      Churned
                    </TabsTrigger>
                  </TabsList>
                      ...
                      <CommandGroup>
                        ...
                        {riskProfileOptions.map((profile) => (
                          ...
                          >
                            ...
                            {selectedRiskProfile.includes(profile) && (
                              <Check className="ml-auto h-4 w-4" />
                            )}
                          </CommandItem>
                        ))}
                  ...
                  {filteredClients.map((c) => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <div className="truncate text-[14px] text-[#1f2937]">
                          {displayName(c)}
                        </div>
                        {!c.original_document_url && (
